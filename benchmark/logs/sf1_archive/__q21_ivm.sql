...
SELECT * FROM rf_orders;REFRESH MATERIALIZED VIEW my_rest.tpch.q21mv;
...
WHERE l_orderkey IN (SELECT okey FROM rf_del);REFRESH MATERIALIZED VIEW my_rest.tpch.q21mv;
...
order by
        count(*) DESC,
        s_name
limit 100;DROP MATERIALIZED VIEW IF EXISTS my_rest.tpch.q21mv;
SELECT concat('sf1,q21,ivm,',(unix_millis(current_timestamp()) - t0.t0s)) AS e2e_csv FROM my_rest.tpch._t0 t0;
EXIT;
